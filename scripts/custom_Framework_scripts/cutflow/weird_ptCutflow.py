...
samples = [
    ...
    "HSCP-Gluino_Par-M-1800_TuneCP5_13p6TeV_madgraphMLM-pythia8_xqcut40_qcut60_PTJ20_NoDupCheck",
]
...
def make_hist(name, title, values, nbins=120, xmin=0.0, xmax=3000.0, normalize=False):
...
def draw_hist_collection(canvas, hists, labels, title, outname, logy=True, legend_coords=(0.52, 0.62, 0.88, 0.88), xline=150.0):
    ...
    nonempty = [h for h in hists if h.Integral() > 0 and h.GetMaximum() > 0]
    if not nonempty:
...
def process_one(sample_name, TriggerMask):
    ...
    pt_trg_HLT_FilterOR_manual = safe_get_branch(arrays, "pt_trg_HLT_FilterOR_manual")
    pt_trg_HLT_FilterOR_saved = safe_get_branch(arrays, "pt_trg_HLT_FilterOR_saved")
    ...
    if TriggerMask == "HLT_MET":
        ...
        trg_defs = [
            ...
            ("HLT_FilterOR_manual", pt_trg_HLT_FilterOR_manual, ROOT.kRed + 1),  #my manual OR
            ("HLT_FilterOR_saved", pt_trg_HLT_FilterOR_saved, ROOT.kBlack),#emerys branch save
            ("PFMET120_PFMHT120_IDTight", pt_trg_PFMET120_PFMHT120_IDTight, ROOT.kMagenta + 1),
            ("PFHT500_PFMET100_PFMHT100_IDTight", pt_trg_PFHT500_PFMET100_PFMHT100_IDTight, ROOT.kOrange + 7),
            ("PFMETNoMu120_PFMHTNoMu120_IDTight_PFHT60", pt_trg_PFMETNoMu120_PFMHTNoMu120_IDTight_PFHT60, ROOT.kCyan + 1),
            ("MET105_IsoTrk50", pt_trg_MET105_IsoTrk50, ROOT.kBlue + 2),
            ("HLT_Mu50", pt_trg_HLT_Mu50, ROOT.kGreen + 1),
            ("HLT_IsoMu24", pt_trg_IsoMu24, ROOT.kSpring + 7),
            ("HLT_IsoMu27", pt_trg_IsoMu27, ROOT.kPink + 1),
            ("HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_DZ", pt_trg_HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_DZ, ROOT.kAzure + 2),
            ("HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL", pt_trg_HLT_Mu23_TrkIsoVVL_Ele12_CaloIdL_TrackIdL_IsoVL, ROOT.kViolet + 2),
        ]
        ...
        trg_defs = [
            ...
            ("HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_DZ", pt_trg_HLT_Mu8_TrkIsoVVL_Ele23_CaloIdL_TrackIdL_IsoVL_DZ, ROOT.kBlue + 2),
