...
def process_file(sample_name, input_file, TriggerMask):
    output_file = (
        f"{output_dir}/"
    ...
    cutflow_steps = {
        ...
        "pt_gen_pair_event_reco_trigger": gen_pair_mask & event_quality_mask & pmet_mask & reco_event_mask & trigger_event_mask,
    }
    ...
    base_cut_mask = gen_pair_mask & event_quality_mask & pmet_mask & reco_event_mask
...
def main():
    for sample_name, input_file in samples.items():
        for TriggerMask in trigger_options:
            process_file(sample_name, input_file, TriggerMask)
