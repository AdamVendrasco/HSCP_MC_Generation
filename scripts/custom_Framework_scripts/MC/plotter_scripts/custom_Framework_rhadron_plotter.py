...
SAMPLE_SETS = {
    "Gluino": {
        "mass_point": 1800,
        "overall_label": "HSCP-Gluino_Par-M-1800_TuneCP5_13p6TeV_NoTrigger",
        "sample_1": {
...
def draw_comparison_with_ratio(
    ...
    normalize=False, ratio_ymin=0.0, ratio_ymax=2.0, vline_x=None):
    set_cms_style()
    ...
    frame.GetYaxis().SetTitleSize(0.045)
    frame.GetYaxis().SetLabelSize(0.040)
    frame.GetYaxis().SetTitleOffset(0.99)
    frame.Draw("AXIS")
    ...
    ratio.GetYaxis().SetTitle("Pythia8/Mg5 (No Bug)")
...
def main():
    base_output_dir = f"plots/NoTrigger/{overall_label}"
    ...
    sample1_dir = os.path.join(
        #trigger_dir,
        f"{sample_name_1}_NoTrigger_{year}_{era}_{tag}_{GTAG}_preselected_events"
    )
    sample2_dir = os.path.join(
        #trigger_dir,
        f"{sample_name_2}_NoTrigger_{year}_{era}_{tag}_{GTAG}_preselected_events"
    )
    comparison_dir = os.path.join(
        #trigger_dir,
        f"comparisons_{overall_label}_NoTrigger_{year}_{era}_{tag}_{GTAG}_preselected_events"
    )
    ...
    if cleaned1 is None or cleaned2 is None:
        print("[SKIP] Skipping comparison — one or both samples have no Events tree")
        return
