...
for MASS in ${MASSES}; do
  for XQCUT in ${XQCUTS}; do
    ...
    sed -i "s/1000021 1.500000e+03 # mgo -> mass of gluino here/1000021 ${MASS_SCI} # mgo -> mass of gluino here/" "${DIR}/${NAME}_param_card.dat"
    sed -i "s/^output HSCPgluinoTemplate -nojpeg$/output ${NAME} -nojpeg/" "${DIR}/${NAME}_proc_card.dat"
