...
# Separate lists for each source
x_rhadron = []
y_rhadron = []
...
# Read data from CSV
with open('qcut_scan.csv', newline='') as csvfile:
...
all_y = y_rhadron + y_stop_1400 + y_stop_525

plt.xticks(range(int(min(all_x)//50) * 50, int(max(all_x)//50 + 1) * 50, 50))
plt.yticks(range(100, int(max(all_y)//50 + 1) * 50, 50))

plt.xlabel('xqcut (GeV)')
plt.ylabel('Number of Accepted Events')
plt.title('Accepted Pythia Events')
plt.grid(True)
plt.legend(bbox_to_anchor=(0.5, 0.2), loc='center left')
plt.tight_layout()
plt.savefig('qcut_scan_plot.png')
